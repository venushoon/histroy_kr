    ...
    timelineData.forEach(item => {
        ...
        if (item.type === 'era') {
            ...
            if (item.type === 'event') { // 단일 사건
                itemContainer.innerHTML = buildEventCard(item);
                ...
                // 각 카드에 클릭 이벤트 추가
                itemContainer.querySelector('.main-card .event-card').addEventListener('click', () => showModal(item.mainEvent));
...
function buildEventCard(data) {
    ...
    return `
        ...
        ${data.type !== 'eventCluster' ? `<div class="timeline-point ${isImportant}"></div>` : ''}
    `;
}
...
function buildRelatedCard(data) {
    return `
        <div class="related-card event-card">
